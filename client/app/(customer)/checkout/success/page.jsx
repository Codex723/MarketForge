            ...
            {TRACKING_STEPS.map((s, i) => {
              ...
              return (
                <div key={s.id} className="flex items-start gap-4 relative">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 z-10 transition-colors ${isActive ? "bg-gold-500/20 border border-gold-500 text-gold-400" : "bg-surface-200 text-stone-600 border border-white/5"}`}>
                    {s.icon}
                  </div>
                  <div className="pt-1">
                    <p className={`text-sm font-semibold ${isActive ? "text-gold-400" : "text-stone-600"}`}>{s.label}</p>
...
// Outer component wraps in Suspense (required by Next.js 14 for useSearchParams)
